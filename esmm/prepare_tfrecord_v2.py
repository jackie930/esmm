...
def _generate(col, cut_off):
    try:
        ...
        if len(b_list) < int(cut_off):
            b_list.extend([-1] * ((cut_off) - len(b_list)))
            return b_list
...
def _generate_weight(col, cut_off):
    try:
        a_list = col.replace('[', '').replace(']', '').replace('\\', '').replace('"', '').split(',')
        b_list = [float(i.split(":")[1]) for i in a_list]
...
def _write_tfrecords(tfrecords_writer):
    while True:
        ...
        df['user_cate_weight'] = df.apply(lambda row: _generate_weight(row['cate'], 3), axis=1)
        df['user_tag_weight'] = df.apply(lambda row: _generate_weight(row['tag'], 3), axis=1)
        df['user_child_weight'] = df.apply(lambda row: _generate_weight(row['child'], 3), axis=1)
        ...
        df['user_tag_favor_1_day_weight'] = df.apply(lambda row: _generate_weight(row['user_tag_favor_1_day'], 3),
                                                     axis=1)
        df['user_tag_favor_7_day_weight'] = df.apply(lambda row: _generate_weight(row['user_tag_favor_7_day'], 3),
                                                     axis=1)
        df['user_tag_favor_15_day_weight'] = df.apply(lambda row: _generate_weight(row['user_tag_favor_15_day'], 3),
                                                      axis=1)
        ...
        for i in range(len(df)):
            try:
                ...
                # item feature
                features['anchor_level'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['anchor_level'][i]]))
                features['anchor_score'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['anchor_score'][i]]))
                ...
                features['i_hot_weekend'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['i_hot_weekend'][i]]))
                features['i_hot_weekday'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['i_hot_weekday'][i]]))
                ...
                features['i_ctr_weekend'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['i_ctr_weekend'][i]]))
                features['i_ctr_weekday'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['i_ctr_weekday'][i]]))
                # 时段偏好(todo: do we need to update encoding style?)
                features['guess_cnt_1d'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['guess_cnt_1d'][i]]))
                features['guess_cnt_7d'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['guess_cnt_7d'][i]]))
                features['guess_cnt_15d'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['guess_cnt_15d'][i]]))
                features['raffle_cnt_1d'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['raffle_cnt_1d'][i]]))
                features['raffle_cnt_7d'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=[df['raffle_cnt_7d'][i]]))
                ...
                features['constellation'] = tf.train.Feature(
                    bytes_list=tf.train.BytesList(
                        value=[str((df['constellation'][i])).encode(encoding='unicode-escape')]))

                # user feature
                features['own_room'] = tf.train.Feature(
                    bytes_list=tf.train.BytesList(
                        value=[str(int(df['own_room'][i])).encode(encoding='unicode-escape')]))
                ...
                features['brand'] = tf.train.Feature(
                    bytes_list=tf.train.BytesList(value=[str(df['brand'][i]).encode()]))
                features['model'] = tf.train.Feature(
                    bytes_list=tf.train.BytesList(value=[str(df['model'][i]).encode()]))
                ...
                features['yuwan_tag'] = tf.train.Feature(
                    bytes_list=tf.train.BytesList(
                        value=[str(int(df['yuwan_tag'][i])).encode(encoding='unicode-escape')]))
                ...
                features['rich_tag'] = tf.train.Feature(
                    bytes_list=tf.train.BytesList(
                        value=[str(int(df['rich_tag'][i])).encode(encoding='unicode-escape')]))
                features['loser_tag'] = tf.train.Feature(
                    bytes_list=tf.train.BytesList(
                        value=[str(int(df['loser_tag'][i])).encode(encoding='unicode-escape')]))
                ...
                features['user_cate_weight'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=df['user_cate_weight'][i]))
                features['user_tag_weight'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=df['user_tag_weight'][i]))
                features['user_child_weight'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=df['user_child_weight'][i]))

                # 偏好分区个数
                features['cate_length'] = tf.train.Feature(
                    int64_list=tf.train.Int64List(value=[int(df['cate_length'][i])]))
                features['tag_length'] = tf.train.Feature(
                    int64_list=tf.train.Int64List(value=[int(df['tag_length'][i])]))
                ...
                features['user_tag_favor_1_day_weight'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=df['user_tag_favor_1_day_weight'][i]))
                features['user_tag_favor_7_day_weight'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=df['user_tag_favor_7_day_weight'][i]))
                features['user_tag_favor_15_day_weight'] = tf.train.Feature(
                    float_list=tf.train.FloatList(value=df['user_tag_favor_15_day_weight'][i]))
...
def write_tfrecords(dataset_dir, save_dir, dataset_flag, writer_process_nums):
    ...
    producer = ESMMDataProducer(dataset_dir=dataset_dir, writer_process_nums=writer_process_nums,
